...
class AskResponse(BaseModel):
    answer: str = Field(..., description="The Tanglish response generated by the chatbot.")
...
async def ask_question(request: AskRequest, db: Session = Depends(get_db)):
    """
    Accepts a user question and returns a generated answer with audio.
    Supports two modes:
    - 'web': Uses Gemini to search and answer questions
    - 'document': Uses document-based RAG (original mode)
    """
    try:
        if not request.question or not request.question.strip():
            raise HTTPException(status_code=400, detail="Question cannot be empty.")

        logger.info(f"Processing question (mode={request.mode}): {request.question}")
...
async def generate_tts_audio(request: TTSRequest):
    ...
    try:
        ...
        if request.voice_settings:
            voice_settings = {
                "stability": request.voice_settings.get("stability", 0.5),
                "similarity_boost": request.voice_settings.get("similarity_boost", 0.75),
                "style": request.voice_settings.get("style", 0.7),
            ...
            voice_settings = {
                "stability": 0.5,   # Balanced emotion & clarity
                "similarity_boost": 0.75,  # Natural voice
                "style": 0.7,   # Moderate emotion for better clarity
