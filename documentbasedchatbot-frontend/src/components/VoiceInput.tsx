...
export const VoiceInput: React.FC<VoiceInputProps> = ({ onTranscription, disabled }) => {
    ...
    const restartTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null); // prevents double-restart

    // Audio level monitor refs
    ...
    useEffect(() => {
        ...
        recognition.continuous = false;  // single-shot per attempt, more reliable
        ...
        recognition.onerror = (event: any) => {
            ...
            if (event.error === 'not-allowed') msg = "Allow mic in browser settings";
        ...
        recognition.onend = () => {
            ...
            if (text) {
                ...
                // Keep trying — restart (prevents double with onerror timer)
                scheduleRestart(recognition);
            } else {
                setIsListening(false);
                stopAudioMonitor();
